...
// ИСПРАВЛЕНИЕ: Импортируем все из нового единого хранилища
...
export const ChatPanel: React.FC<ChatPanelProps> = ({ sessionId }) => {
	// ИСПРАВЛЕНИЕ: Получаем состояние и экшены из useAppStore
				...
				{session.messages.length === 0 ? (
					...
					session.messages.map((msg: Message) => (
						<Paper 
							key={msg.id} 
							elevation={1} 
							sx={{ p: 1.5, mb: 1.5, bgcolor: msg.role === 'user' ? 'primary.light' : 'background.paper' }}
						>
							<Typography variant="body2" sx={{ fontWeight: 'bold', textTransform: 'uppercase' }}>{msg.role} (Turn: {msg.turn})</Typography>
							<Typography variant="body1" sx={{ whiteSpace: 'pre-wrap', py: 1 }}>{msg.content}</Typography>
							<Typography variant="caption" color="text.secondary">Tokens: {msg.tokenCount}</Typography>
						</Paper>
					))
				)}
			</Box>
